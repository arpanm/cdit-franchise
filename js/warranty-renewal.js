...
document.getElementById('sendWarrantyPaymentLink').addEventListener('click', function() {
    ...
    if (selectedPlan) {
        ...
        // Update warranty status in the warranty card
        const warrantyCard = document.querySelector(`[data-warranty-id="${currentWarrantyId}"]`);
        if (warrantyCard) {
            const statusBadge = warrantyCard.querySelector('.warranty-status');
            if (statusBadge) {
                statusBadge.innerHTML = '<span class="badge bg-warning">Payment Pending</span>';
            }
        }
...
// Variable to store the current warranty being renewed
let currentWarrantyId = null;

// Function to handle warranty renewal button click
function handleWarrantyRenewal(warrantyId) {
    currentWarrantyId = warrantyId;
    const modal = new bootstrap.Modal(document.getElementById('extendedWarrantyModal'));
    modal.show();
}