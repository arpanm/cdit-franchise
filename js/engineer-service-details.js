...
function displaySearchResults(results) {
    const container = document.getElementById('itemSelectionCards');
    if (!container) {
        console.error('Error: itemSelectionCards element not found');
        return;
    }
    container.innerHTML = '';
    ...
    results.forEach(item => {
        ...
        card.className = 'card h-100 selectable-card';
        ...
        cardBody.innerHTML = `
            ...
            ${item.source ? `<p class="card-text mb-2">Inventory: ${item.source} : ${item.inventory}</p>` : ''}
            <div class="form-check">
                <input class="form-check-input" type="checkbox" value="${item.id}" id="check_${item.id}" onclick="event.stopPropagation();">
...
function addSelectedItems() {
    const selectedCheckboxes = document.querySelectorAll('#itemSelectionCards input[type="checkbox"]:checked');
...
function displayAddedItemsCard() {
    const container = document.getElementById('addedItemsContainer');
    if (!container) {
        console.error('Error: addedItemsContainer element not found');
        return;
    }
    container.innerHTML = ''; // Clear existing content

    // Add the service request card first
    const serviceCol = document.createElement('div');
    serviceCol.className = 'col-12 col-md-6 col-lg-4 mb-3';
    serviceCol.innerHTML = `
        <div class="card h-100 shadow-sm">
            <div class="card-body d-flex flex-column">
                <h6 class="card-title mb-3">Repair Request</h6>
                <div class="mb-auto">
                    <p class="card-text mb-2"><small class="text-muted">ID: I001</small></p>
                    <p class="card-text mb-2">Category: Service</p>
                    <p class="card-text">Price: Free Service</p>
                </div>
                <span class="badge bg-success mt-2">Free</span>
            </div>
        </div>
    `;
    container.appendChild(serviceCol);
        ...
        col.innerHTML = `
            <div class="card h-100 shadow-sm">
                <div class="card-body d-flex flex-column">
                    ...
                    <div class="mb-auto">
                        <p class="card-text mb-2"><small class="text-muted">ID: ${item.id}</small></p>
                        <p class="card-text mb-2">Category: ${item.category}</p>
                        <p class="card-text">Price: ₹${item.price}</p>
                    </div>
                    <div class="d-flex flex-wrap gap-2 mt-3">
                        ${item.paymentStatus === 'Pending' ? 
                            `<button class="btn btn-sm btn-danger flex-grow-1" onclick="removeItem('${item.id}')">Remove</button>
                            <button class="btn btn-sm btn-primary flex-grow-1" onclick="markPaid('${item.id}')">Mark Paid by Cash</button>` :
                                ...
                                item.paymentStatus === 'Paid' ?
                                    item.source === 'Franchise' ? 
                                        `<div class="d-flex align-items-center gap-2 w-100">
                                            <span class="text-muted flex-grow-1">${item.source ? item.source + " : " + item.inventory : ""}</span>
                                            <button class="btn btn-sm btn-primary" onclick="markDelivered('${item.id}')">Mark Delivered</button>
                                        </div>` :
                                        `<span class="text-muted">${item.source ? item.source + " : " + item.inventory + " : <a href='#' class='text-primary'>Status: Pending</a>" : ""}</span>` : 
                                        `<span class="text-muted">${item.source? item.source + " : " + item.inventory : ""}</span>`}
                        <button class="btn btn-sm btn-outline-danger" onclick="removeItem('${item.id}')"><i class="pa pas-cross"></i></button>
                    </div>
                </div>
            </div>
        `;
...
// Function to display added items
function displayAddedItemsOld() {
    const tbody = document.getElementById('addedItemsTable');
    if (!tbody) {
        console.error('Error: addedItemsTable element not found in the DOM. Make sure the table exists and the function is called after DOM is loaded.');
        return;
    }
